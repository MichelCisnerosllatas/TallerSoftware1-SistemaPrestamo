...
CREATE VIEW `v_prestamolistar` AS
    select 
        ...
        count(`p`.`IdPago`) AS `CuotasPagadas`,
        sum(`p`.`MontoPago`) AS `SumaTotalPagos`,
    ...
    from
        ...
        from
            (`usuario` `u`
        left join `persona` `p` ON ((`u`.`IdPersona` = `p`.`IdPersona`)))) `u` ON ((`ue`.`IdUsuario` = `u`.`IdUsuario`)))
        left join (select 
            `c`.`IdCliente` AS `IdCliente`,
        ...
        from
            (`cliente` `c`
        left join `persona` `p` ON ((`c`.`IdPersona` = `p`.`IdPersona`)))) `c` ON ((`pre`.`IdCliente` = `c`.`IdCliente`)))
        left join `pagos` `p` ON ((`pre`.`IdPrestamo` = `p`.`IdPrestamo`)))
    group by `pre`.`IdPrestamo` , `u`.`IdPersonaUsuario` , `ue`.`IdUsuario` , `u`.`UsuarioRegistro` , `ue`.`IdUserEmpresa` , `ue`.`IdEmpresa` , `e`.`NombreEmpresa` , `c`.`IdPersonaCliente` , `c`.`IdCliente` , `c`.`Cliente` , `tp`.`NombreTipoPago` , `pre`.`FechaPago` , `pre`.`MontoInteres` , `pre`.`PorcentajeInteres` , `pre`.`MontoCalculado` , `pre`.`MontoDevolucion` , `pre`.`MontoPrestado` , `pre`.`Cuotas` , `pre`.`MontoCuota` , `pre`.`ObservacionPrestamo` , `pre`.`Estado` , `pre`.`FechaRegistro` , `pre`.`FechaModifica` , `pre`.`FechaVencimiento`
...
BEGIN
	IF EXISTS (SELECT * FROM PAGOS WHERE Idpago = P_IdPago) THEN		
		UPDATE pagos SET Estado = "Anulado", FechaAnulado = P_FechaAnulado, UsuarioAnulado = P_UsuarioAnulado
		WHERE IdPago = P_IdPago;
		
		IF ROW_COUNT() > 0 THEN
			SET Mensaje = "Anulado Correctamente";
			SET Success = 1;
		ELSE
			SET Mensaje = "No Se Anuló el Pago";
			SET Success = 0;
		END IF;
	ELSE
		SET Success = 0;
...
BEGIN
	...
	IF row_count() > 0 THEN
        ...
        UPDATE userempresa set IdEmpresa = sp_idEmpresa;
