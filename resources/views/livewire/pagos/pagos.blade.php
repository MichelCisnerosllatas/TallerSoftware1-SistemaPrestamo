<div>
    <div class="div_text_tituloPago">
        <div>
            ...
            {{--            <label>Total {{ $totalRegistros }} Pagos en {{ $totalPagina }} paginas</label>--}}
        </div>
    ...
    @if(count($datosPagos) > 0)
        <div class="divtablaPago">
            <table>
                ...
                <tbody>
                @foreach($datosPagos as $index => $Pago)
                    <tr>
                        <td> {{ $Pago['IdPago'] }}</td>
                        <td>{{ $Pago['Cliente'] }}</td>
                        <td>S/. {{ $Pago['MontoPrestado'] }}</td>
                        <td>{{ $Pago['PorcentajeInteres'] }}%</td>
                        <td>S/. {{ $Pago['MontoCalculado'] }}</td>
                        <td>{{ $Pago['Cuotas'] }}</td>
